...
export default async function AdminLogsPage() {
  ...
  const formatDetails = (details: any, resource: string | null) => {
    ...
    if (resource?.startsWith("banner_")) {
      ...
      if (details.display_mode) {
        ...
        parts.push(
          `版面：${map[details.display_mode] || details.display_mode}`
        );
      }
  ...
  const getActionColor = (action: string) => {
    ...
    if (action.includes("SETTING") || action.includes("BANNER"))
      return "outline"; // System/Content related
    return "outline";
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">查看系統所有重要操作紀錄</p>
      </div>
